...
本文对应当前固件配置（`config.h`）：
- 主固件已启用：OLED、EC11、对射式红外人体存在传感器、GY-302 BH1750、语音模块、蜂鸣器
- 使用板载 WS2812 作为视觉指示（`WS2812_PIN=48`）

说明：
- 对射式红外人体存在传感器和 `GY-302 BH1750` 已按整机接线正式接入主固件
...
| EC11 | 3V3 | + | 编码器供电 |
| EC11 | GND | GND | 共地 |
| 对射式红外 | GPIO4 | DO/OUT | 数字输出，挡住光路表示有人 |
| 对射式红外 | 3V3 或 5V | VCC | 按模块规格供电 |
| 对射式红外 | GND | GND | 共地 |
...
- 对射式红外输出通常可直接接 ESP32 数字输入；当前固件按 `LOW=挡住/有人` 解释输入电平，如模块极性不同需同步修改 `PERSON_SENSOR_ACTIVE_LEVEL`。
- 主固件当前按对射式实时状态判定人体在位，不使用 PIR 预热与保持时间。
- 如果设备长期显示“有人”，优先检查 `VCC/GND/DO` 是否接对、是否与 ESP32 共地，以及光路是否真的处于未遮挡状态；确认接线无误后再检查 `PERSON_SENSOR_ACTIVE_LEVEL` 是否与模块极性一致。
- 主固件中：BH1750 lux 会参与 OLED 显示与 MQTT `ambientLux` 上报。
- 主固件中：人体存在传感器会参与 `personPresent` 判定，并在无人时抑制坐姿提醒。
...
### 4.1 HC-SR501 Demo（保留为历史排障 Demo）
...
- 历史 HC-SR501 Demo 仍使用 `GPIO4`，因此可继续用于验证该输入口是否可读
