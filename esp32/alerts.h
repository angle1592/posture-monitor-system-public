...
inline void _alertsSetIndicatorColor(uint8_t r, uint8_t g, uint8_t b) {
#if STATUS_LED_PIN >= 0
    bool on = (r | g | b) != 0;
...
inline void alerts_setAlertMode(uint8_t modeMask) {
    _alertModeMask = modeMask;
}

inline uint8_t alerts_getAlertMode() {
    return _alertModeMask;
}

inline void alerts_setColor(uint8_t r, uint8_t g, uint8_t b) {
    _alertsSetIndicatorColor(r, g, b);
}

inline void alerts_lockIndicator(bool lock, uint8_t r, uint8_t g, uint8_t b) {
...
inline void alerts_triggerBuzzerPulse(unsigned long durationMs) {
#if ENABLE_BUZZER
...
inline void alerts_beep(int durationMs) {
    alerts_triggerBuzzerPulse((unsigned long)durationMs);
}

inline bool alerts_voiceEnabled() {
    return (_alertModeMask & ALERT_MODE_VOICE) != 0;
}

inline void alerts_update(bool mqttConnected, bool k230Valid, bool isAbnormal, bool noPerson) {
#if ENABLE_BUZZER
